...
fun GameBoard<Int?>.moveValues(direction: Direction): Boolean {
    val cells = when (direction) {
        Direction.UP -> (1..this.width).map { j -> (1..this.width).map { i -> Cell(i,j) } }
        Direction.DOWN -> (1..this.width).map { j -> (1..this.width).reversed().map { i -> Cell(i,j) } }
        Direction.LEFT -> (1..this.width).map { i -> (1..this.width).map { j -> Cell(i,j) } }
        Direction.RIGHT -> (1..this.width).map { i -> (1..this.width).reversed().map { j -> Cell(i,j) } }
    }
